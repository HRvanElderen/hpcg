                   ...
                   const TestCGData &testcg_data, const TestSymmetryData &testsymmetry_data, const TestNormsData &testnorms_data, int global_failure, bool quickPath)
{
  ...
  { // Only PE 0 needs to compute and report timing results
    ...
    double fnnz_Af = Af->totalNumberOfNonzeros;
    double fnrow_Af = Af->totalNumberOfRows;
    fnreads_precond += fniters * (2.0 * fnnz_Af * (sizeof(double) + sizeof(local_int_t)) + fnrow_Af * sizeof(double));
    ...
    json_dict["procs"] = A.geom->size;
    json_dict["main_times"] = {
        {"setup", times[9]},
        {"total", times[0]},
        {"DDOT", times[1]},
        {"WAXPBY", times[2]},
        ...
        {"opt_time", times[7]},
        {"ref_time", times[8]}};
    json_dict["mean_times"] = {
        {"setup", mean_times[9]},
        {"total", mean_times[0]},
        {"DDOT", mean_times[1]},
        {"WAXPBY", mean_times[2]},
        {"SPMV", mean_times[3]},
        {"allreduce", mean_times[4]},
        {"MG", mean_times[5]},
        {"halo_time", mean_times[6]},
        {"opt_time", mean_times[7]},
        {"ref_time", mean_times[8]}};
    json_dict["max_times"] = {
        {"setup", max_times[9]},
        {"total", max_times[0]},
        {"DDOT", max_times[1]},
        {"WAXPBY", max_times[2]},
        {"SPMV", max_times[3]},
        {"allreduce", max_times[4]},
        {"MG", max_times[5]},
        {"halo_time", max_times[6]},
        {"opt_time", max_times[7]},
        {"ref_time", max_times[8]}};
    json_dict["nr_equations"] = A.totalNumberOfRows;
    json_dict["non_zeors"] = A.totalNumberOfNonzeros;
    // json_dict["multigrid_data"] = mg_data;
    json_dict["geometry"] = {{"npx", A.geom->npx},
                             {"npy", A.geom->npy},
                             {"npz", A.geom->npz},
                             {"gnx", A.geom->gnx},
                             {"gny", A.geom->gny},
                             ...
                             {"ny", A.geom->ny},
                             {"nz", A.geom->nz}};
    json_dict["iter_data"] = {{"ref_iters_set", refMaxIters},
                              {"opt_iters_set", optMaxIters},
                              {"ref_iters_total", refMaxIters * numberOfCgSets},
                              {"opt_iters_total", optMaxIters * numberOfCgSets}};
    json_dict["reproducibility_data"] = {{"mean", testnorms_data.mean}, {"var", testnorms_data.variance}};
    json_dict["flops"] = {{"DDOT", fnops_ddot},
                          {"WAXPBY", fnops_waxpby},
                          {"SpMV", fnops_sparsemv},
                          {"MG", fnops_precond},
                          {"Total", fnops},
                          {"Total_conv", frefnops}};
    // times = main_times
    json_dict["GB/s"] = {{"Read", (fnreads / times[0] / 1.0E9)},
                         {"Write", (fnwrites / times[0] / 1.0E9)},
                         {"Total", (fnreads + fnwrites) / (times[0]) / 1.0E9},
                         {"Total_conv_opt", (frefnreads + frefnwrites) / (times[0] + fNumberOfCgSets * (times[7] / 10.0 + times[9] / 10.0)) / 1.0E9}};

    double totalGflops = frefnops / (times[0] + fNumberOfCgSets * (times[7] / 10.0 + times[9] / 10.0)) / 1.0E9;
    json_dict["GFLOP/s"] = {{"DDOT", (fnops_ddot / times[1] / 1.0E9)},
                            {"WAXPBY", (fnops_waxpby / times[2] / 1.0E9)},
                            {"SpMV", (fnops_sparsemv / times[3] / 1.0E9)},
                            {"MG", (fnops_precond / times[5] / 1.0E9)},
                            {"Total", (fnops / times[0] / 1.0E9)},
                            {"Total_conv", (frefnops / times[0] / 1.0E9)},
